...
// esp_err_t spi_master_write(nfc4_t *ctx, uint8_t tx_buff[], uint8_t length)
// {
//     spi_transaction_t t;
//     memset(&t, 0, sizeof(t));
...
//     printf("\n");
//     printf("SPI_WRITE => Length: %d \n", length);
//     t.length = 8 * length;

//     for (int i = 0; i < length; i++)
//     {
//         printf("SPI_WRITE => TX_BUFF[%d]: 0x%x \n", i, tx_buff[i]);
...
esp_err_t spi_master_write(nfc4_t *ctx, uint8_t tx_buff[], uint8_t length)
{
    ...
    for (int i = 0; i < length; i++)
    {
        printf("SPI_WRITE => TX_DATA[%d]: 0x%x \n", i, tx_buff[i]);
    }
    printf("\n");
...
esp_err_t spi_master_write_then_read(nfc4_t *ctx, uint8_t tx_buff[], uint8_t length, uint8_t *data_out, uint8_t len)
{
    uint8_t tx_rx_len = length + len;

    uint8_t temp[tx_rx_len];
    ...
    // for (int i = 0; i < tx_rx_len; i++)
    // {
    //     printf("SPI_READ => ALL_RX_DATA[%d]: 0x%x \n", i, temp[i]);
    // }

    uint8_t temp2[len];

    int j = 0;
    for (uint8_t cnt = length; cnt < tx_rx_len; cnt++)
    {
        printf("SPI_READ => RX_DATA[%d]: 0x%x \n", cnt, temp[cnt]);
        temp2[j] = temp[cnt];
        j++;
    ...
    memcpy(data_out, temp2, sizeof(temp2));
    return ESP_OK;
}
...
esp_err_t nfc4_init(nfc4_t *ctx, nfc4_cfg_t *cfg)
{
    ...
    ctx->spi = handle;
    ctx->chip_select = GPIO_CS;
    ctx->irq = GPIO_HANDSHAKE;
...
esp_err_t rmr(nfc4_t *ctx, uint8_t reg, uint8_t *data_out, uint8_t len)
{
    ...
    for (uint8_t cnt = 1; cnt < len + 1; cnt++)
    {
        printf("Data_Out[%d]: 0x%x \n", cnt, temp[cnt]);
        data_out[cnt - 1] = temp[cnt];
    }
...
esp_err_t nfc4_default_cfg(nfc4_t *ctx)
{
    ...
    ESP_LOGW(TAG2, "nfc4_send_direct_command() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
    ...
    ESP_LOGW(TAG2, "nfc4_write_register() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    nfc4_check_chip_id(ctx, NULL);
    ESP_LOGW(TAG2, "nfc4_write_register() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
    ...
    ESP_LOGW(TAG2, "nfc4_set_register_bits() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    /* Disable internal overheat protection */
    error_flag |= nfc4_modify_test_register_bits(ctx, 0x04, 0x10, 0x10);
    ESP_LOGW(TAG2, "nfc4_modify_test_register_bits() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    /* Enable Oscillator and wait until it gets stable */
    error_flag |= nfc4_enable_osc(ctx);
    ESP_LOGW(TAG2, "nfc4_enable_osc() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
    ...
    ESP_LOGW(TAG2, "nfc4_clear_register_bits() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    /* After reset all interrupts are enabled, so disable them at first */
    error_flag |= nfc4_disable_interrupt(ctx, 0x00FFFFFFFF);
    ESP_LOGW(TAG2, "nfc4_disable_interrupt() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    /* And clear them, just to be sure */
    error_flag |= nfc4_clear_interrupts(ctx);
    ESP_LOGW(TAG2, "nfc4_clear_interrupts() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    /* Disable any previous observation mode */
    error_flag |= nfc4_write_test_register(ctx, 0x01, 0x40);
    ESP_LOGW(TAG2, "nfc4_write_test_register() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    /* Apply RF Chip generic initialization */
    nfc4_initialize_analog_config(ctx, &nfc4_analog_config_settings);
    ESP_LOGW(TAG2, "nfc4_initialize_analog_config() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    nfc4_set_analog_config(ctx, NFC4_ANALOG_CONFIG_CHIP_INIT);
    ESP_LOGW(TAG2, "nfc4_set_analog_config() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
    ...
    ESP_LOGW(TAG2, "nfc4_modify_register_bits() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
    ...
    ESP_LOGW(TAG2, "nfc4_read_register() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
    ...
    if (NFC4_REGULATOR_CTRL_REG_S != (reg_data & NFC4_REGULATOR_CTRL_REG_S))
    {
        ...
        error_flag |= nfc4_send_direct_command_with_result(ctx, NFC4_CMD_ADJUST_REGULATORS,
                                                           ...
                                                           10000,
                                                           &reg_data);
        ESP_LOGW(TAG2, "nfc4_send_direct_command_with_result() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
    }
...
{
    return nfc4_spi_write(ctx, reg, data_in, len);
}

esp_err_t nfc4_generic_read(nfc4_t *ctx, uint8_t reg, uint8_t *data_out, uint16_t len)
{
    return nfc4_spi_read(ctx, reg, data_out, len);
}
...
esp_err_t nfc4_send_direct_command_with_result(nfc4_t *ctx, uint8_t cmd, uint8_t result_reg, uint32_t timeout, uint8_t *result)
{
    ...
    ESP_LOGW(TAG2, "nfc4_check_for_received_interrupts() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    nfc4_get_interrupt(ctx, NFC4_IRQ_MASK_DCT);
    ESP_LOGW(TAG2, "nfc4_get_interrupt() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    error_flag |= nfc4_enable_interrupt(ctx, NFC4_IRQ_MASK_DCT);
    ESP_LOGW(TAG2, "nfc4_enable_interrupt() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    error_flag |= nfc4_generic_write(ctx, cmd | NFC4_MODE_DIRECT_COMMAND, 0, 0);
    ESP_LOGW(TAG2, "nfc4_generic_write() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);

    nfc4_wait_for_interrupt(ctx, NFC4_IRQ_MASK_DCT, timeout);
    ESP_LOGW(TAG2, "nfc4_wait_for_interrupt() %s:%d STATUS: %d", __FILE__, __LINE__, error_flag);
...
esp_err_t nfc4_enable_interrupt(nfc4_t *ctx, uint32_t mask)
{
    uint8_t rx_buf[4] = {0};
    uint8_t tx_data = 0;
    esp_err_t error_flag = ESP_OK;

    nfc4_read_multiple_registers(ctx, NFC4_REG_IRQ_MASK_MAIN, rx_buf, 4);
    ...
    for (uint8_t cnt = 0; cnt < 4; cnt++)
    {
        ...
        if (0 != tx_data)
        {
            ...
            ESP_LOGI(TAG2, "%s:%d CNT: %d tx_data: 0x%x", __FILE__, __LINE__, cnt, tx_data);
...
//     uint32_t rxBuff3 = 0;
//     uint32_t rxBuff4 = 0;
//     uint32_t intSts = 0x00000000;

//     while (nfc4_get_irq_pin(ctx))
//     {

//         error_flag |= nfc4_read_register(ctx, 0x1A, &rxBuff1);
//         error_flag |= nfc4_read_register(ctx, 0x1B, &rxBuff2);
//         error_flag |= nfc4_read_register(ctx, 0x1C, &rxBuff3);
//         error_flag |= nfc4_read_register(ctx, 0x1D, &rxBuff4);

//         ESP_LOGI(TAG2, "rxBuff1=0x%lx\trxBuff2=0x%lx\trxBuff3=0x%lx\trxBuff4=0x%lx", rxBuff1, rxBuff2, rxBuff3, rxBuff4);
...
//         ESP_LOGI(TAG2, "INT_STATUS: 0x%lx", intSts);

//         ctx->int_status |= intSts;
//     }
...
esp_err_t nfc4_check_for_received_interrupts(nfc4_t *ctx)
{
    ...
    while (nfc4_get_irq_pin(ctx))
    {
        ...
        ESP_LOGI(TAG2, "rxBuff1=0x%x\trxBuff2=0x%x\trxBuff3=0x%x\trxBuff4=0x%x", rx_buf[0], rx_buf[1], rx_buf[2], rx_buf[3]);
        ...
        ESP_LOGI(TAG2, "nfc4_check_for_received_interrupts() %s:%d FINAL STATUS: %lx ", __FILE__, __LINE__, ctx->int_status);
    }
...
uint32_t nfc4_get_interrupt(nfc4_t *ctx, uint32_t mask)
{
    ...
    ESP_LOGW(TAG2, "nfc4_get_interrupt() %s:%d int_status & mask: 0x%lx", __FILE__, __LINE__, status);
...
esp_err_t nfc4_check_chip_id(nfc4_t *ctx, uint8_t *revision)
{
    ...
    ESP_LOGI(TAG2, "=========================");
    ESP_LOGI(TAG2, "====== CHIP ID: 0x%x ====", chip_id);
    ESP_LOGI(TAG2, "=========================");
...
esp_err_t nfc4_enable_osc(nfc4_t *ctx)
{
    ...
    if (NFC4_OP_CTRL_EN != (reg_data & NFC4_OP_CTRL_EN))
    {
        ...
        nfc4_wait_for_interrupt(ctx, NFC4_IRQ_MASK_OSC, 10000);
        ESP_LOGW(TAG2, "nfc4_wait_for_interrupt() %s:%d", __FILE__, __LINE__);
    ...
    {
        ESP_LOGI(TAG2, "Xtal oscillation is stable");
        return error_flag;
    }
...
{
    uint8_t getIrq = digital_in_read(ctx->irq);
    printf("nfc4_get_irq_pin %s:%d IRQ LEVEL: %d \n", __FILE__, __LINE__, getIrq);
    return getIrq;
}
...
esp_err_t nfc4_get_mifare_tag_uid(nfc4_t *ctx, uint8_t *uid, uint8_t *uid_len)
{
    ...
    nfc4_rfal_nfc_worker(ctx);
    ESP_LOGW(TAG2, "nfc4_rfal_nfc_worker() %s:%d", __FILE__, __LINE__);
...
static esp_err_t nfc4_rfal_nfca_poller_initialize(nfc4_t *ctx)
{
    ...
    ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_initialize()  %s:%d Returned Error Code: %d", __FILE__, __LINE__, error_flag);
    return error_flag;
}
                                                      ...
                                                      nfc4_rfal_nfca_sens_res_t *sens_res)
{
    ...
    ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_check_presence()  %s:%d Returned Error Code: %d", __FILE__, __LINE__, error_flag);
    return error_flag;
}
...
static esp_err_t nfc4_rfal_nfca_poller_get_single_collision_resolution_status(nfc4_t *ctx)
{
    ...
    if (ctx->nfca.col_res.cascade_lvl > (uint8_t)NFC4_NFCA_SEL_CASCADE_L3)
    {
        ...
        ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_single_collision_resolution_status()  %s:%d Returned Error Code: NFC4_RFAL_ERR_INTERNAL", __FILE__, __LINE__);
    ...
    case NFC4_NFCA_CR_SDD: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall through */
    {
        ...
        if (NFC4_RFAL_ERR_RF_COLLISION == error_flag)
        {
            ...
            {
                ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_single_collision_resolution_status()  %s:%d Returned Error Code: NFC4_RFAL_ERR_PROTO", __FILE__, __LINE__);
                return NFC4_RFAL_ERR_PROTO;
            }
            ...
            if (!(*ctx->nfca.col_res.coll_pend))
            {
                ...
                *ctx->nfca.col_res.coll_pend = true;
                ...
                ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_single_collision_resolution_status()  %s:%d Returned Error Code: NFC4_RFAL_ERR_IGNORE", __FILE__, __LINE__);
        ...
        if (NFC4_RFAL_ERR_NONE != error_flag)
        {
            ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_single_collision_resolution_status()  %s:%d Returned Error Code: %d", __FILE__, __LINE__, error_flag);
            return error_flag;
        }
    ...
    {
        ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_single_collision_resolution_status()  %s:%d Returned Error Code: NFC4_RFAL_ERR_NONE", __FILE__, __LINE__);
        return NFC4_RFAL_ERR_NONE;
    }
    default:
    {
        ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_single_collision_resolution_status()  %s:%d Returned Error Code: NFC4_RFAL_ERR_WRONG_STATE", __FILE__, __LINE__);
        return NFC4_RFAL_ERR_WRONG_STATE;
    }
    }

    ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_single_collision_resolution_status()  %s:%d Returned Error Code: NFC4_RFAL_ERR_BUSY", __FILE__, __LINE__);
...
{
    esp_err_t error_flag;
    ESP_LOGW(TAG2, "=== INIT === nfc4_rfal_nfca_poller_get_full_collision_resolution_status() FILE: %s:%d", __FILE__, __LINE__);
...
static esp_err_t nfc4_rfal_nfc_poll_coll_resolution(nfc4_t *ctx)
{
    ...
    ESP_LOGW(TAG2, "nfc4_rfal_nfc_poll_coll_resolution() %s:%d ", __FILE__, __LINE__);

    if (ctx->nfc_dev.dev_cnt >= 1)
    {
        ESP_LOGW(TAG2, "%s:%d ctx->nfc_dev.dev_cnt >= 1:%d", __FILE__, __LINE__, ctx->nfc_dev.dev_cnt >= 1);
    ...
    {
        ESP_LOGW(TAG2, "!ctx->nfc_dev.is_tech_init %s:%d ", __FILE__, __LINE__);

        EXIT_ON_ERR(error_flag, nfc4_rfal_nfca_poller_initialize(ctx)); /* Initialize RFAL for NFC-A */
        ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_initialize() %s:%d ", __FILE__, __LINE__);

        EXIT_ON_ERR(error_flag, nfc4_rfal_field_on(ctx)); /* Turns the Field On */
        ESP_LOGW(TAG2, "nfc4_rfal_field_on() %s:%d ", __FILE__, __LINE__);
    ...
    {
        ESP_LOGW(TAG2, "!ctx->nfc_dev.is_oper_ongoing %s:%d", __FILE__, __LINE__);

        EXIT_ON_ERR(error_flag, nfc4_rfal_nfca_poller_start_full_collision_resolution(ctx, nfca_dev_list, &dev_cnt));
        ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_start_full_collision_resolution() FILE: %s:%d ", __FILE__, __LINE__);
    ...
    ESP_LOGW(TAG2, "nfc4_rfal_nfca_poller_get_full_collision_resolution_status() FILE: %s:%d  ERROR_FLAG: %d", __FILE__, __LINE__, error_flag);
...
static void nfc4_rfal_nfc_worker(nfc4_t *ctx)
{
    ...
    {
        ESP_LOGW(TAG2, "NFC4_NFC_STATE_START_DISCOVERY nfc4_rfal_nfc_worker() %s:%d", __FILE__, __LINE__);
    ...
    {
        ESP_LOGW(TAG2, "NFC4_NFC_STATE_POLL_COLAVOIDANCE nfc4_rfal_nfc_worker() %s:%d", __FILE__, __LINE__);

        /* Resolve any eventual collision. */
        error_flag = nfc4_rfal_nfc_poll_coll_resolution(ctx);
        ESP_LOGW(TAG2, "nfc4_rfal_nfc_poll_coll_resolution() %s:%d ERROR_FLAG: %d", __FILE__, __LINE__, error_flag);

        /* Wait until all technologies are performed. */
        if (NFC4_RFAL_ERR_BUSY != error_flag)
        {
            ESP_LOGW(TAG2, "==== FIRST IF === nfc4_rfal_nfc_poll_coll_resolution() %s:%d ERROR_FLAG: %d", __FILE__, __LINE__, error_flag);
            /* Check if any error occurred or no devices were found. */
            if ((NFC4_RFAL_ERR_NONE != error_flag) || (0U == ctx->nfc_dev.dev_cnt))
            {
                ESP_LOGW(TAG2, "==== SECOND IF === nfc4_rfal_nfc_poll_coll_resolution() %s:%d ERROR_FLAG: %d", __FILE__, __LINE__, error_flag);
    ...
    {
        ESP_LOGW(TAG2, "NFC4_NFC_STATE_POLL_ACTIVATION nfc4_rfal_nfc_worker() %s:%d", __FILE__, __LINE__);

        error_flag = nfc4_rfal_nfc_poll_activation(ctx, ctx->nfc_dev.sel_dev_idx);
        ESP_LOGW(TAG2, "nfc4_rfal_nfc_poll_activation() %s:%d  ERROR_FLAG: %d", __FILE__, __LINE__, error_flag);
...
static esp_err_t nfc4_set_analog_config(nfc4_t *ctx, uint16_t config_id)
{
    ...
    {
        ESP_LOGE(TAG2, "ANALOG CONFIG MGMT NOT READY %s:%d", __FILE__, __LINE__);
        return NFC4_ERROR;
    }
...
static esp_err_t nfc4_spi_write(nfc4_t *ctx, uint8_t reg, uint8_t *data_in, uint16_t len)
{
    ...
    else if (reg & NFC4_REG_SPACE_B_MASK)
    {
        ...
        error_flag = spi_master_write(ctx, tx_buf, len + 2);
    }
    else
    {
        tx_buf[0] = reg;
...
static esp_err_t nfc4_spi_read(nfc4_t *ctx, uint8_t reg, uint8_t *data_out, uint16_t len)
{
    ...
    if (NFC4_CMD_TEST_ACCESS == reg)
    {
        ...
        error_flag = spi_master_write_then_read(ctx, tx_buf, 2, data_out, len);
    }
    else if (reg & NFC4_REG_SPACE_B_MASK)
    {
        tx_buf[0] = NFC4_CMD_SPACE_B_ACCESS;
    ...
    else if ((NFC4_MODE_PT_MEM_READ != reg) && (NFC4_MODE_FIFO_READ != reg))
    {
        ...
        error_flag = spi_master_write_then_read(ctx, tx_buf, 1, data_out, len);
    }
    else
    {
        tx_buf[0] = reg;
